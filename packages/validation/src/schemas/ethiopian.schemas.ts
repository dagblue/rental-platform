 ...
 */
export const ethiopianPhoneSchema = z.string()
  .transform((phone, ctx): EthiopianPhoneNumber => {
    ...
    const provider = prefix === '9' ? 'ETHIO_TELECOM' : 
                     prefix === '7' ? 'SAFARICOM' : 'OTHER';
...
export const mobileMoneyPaymentSchema = z.object({
  ...
  amount: z.number()
    ...
    .min(1, 'Minimum payment is 1 ETB'),
...
export const trustLevelUpgradeSchema = z.object({
  ...
  })).min(1, 'At least one guarantor is required').max(5, 'Maximum 5 guarantors allowed')
});
